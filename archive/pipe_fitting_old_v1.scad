...
color("pink") difference() {
  union() {
    ...
    translate([0,0,thread_length]) cylinder(middle_section_distance, middle_section_outer_dia/2, middle_section_outer_dia/2);
    
    translate([0,0,middle_section_distance+thread_length+thread_length]) rotate([180,0,0]) ScrewThread(fitting_total_outer_dia, thread_length, tolerance=thread_tol, tip_height=2, tip_min_fract=3/4);
  }

  translate([0,0,0]) union() {
    translate([0,0,-z_fite]) cylinder(gasket_seat_depth+z_fite, (pipe_fit_dia+gasket_seat_width * 2)/2, pipe_fit_dia/2);

    translate([0,0,gasket_seat_depth]) cylinder(pipe_fit_length, pipe_fit_dia/2, pipe_fit_dia/2);

    translate([0,0,gasket_seat_depth+pipe_fit_length+gasket_seat_depth+z_fite]) rotate([180,0,0]) cylinder(gasket_seat_depth+z_fite, (pipe_fit_dia+gasket_seat_width*2)/2, pipe_fit_dia/2);
  }
}
...
// Nuts
color("blue") translate([0,0,middle_section_distance+thread_length+thread_length+offset1]) 
union() {
  difference() {
...
color("blue") translate([0,0,-offset1]) rotate([180,0,0]) 
union() {
  difference() {
    MetricNut(fitting_total_outer_dia, tolerance=thread_tol);
    translate([0,0,-z_fite]) cylinder(nut_circle_depression+z_fite,fitting_total_outer_dia/2+nut_circle_depression*2,fitting_total_outer_dia/2+nut_circle_depression*2);
...
offset2 = 4 * (gasket_depth-gasket_seat_depth);
offset3 = 1.5 * fitting_total_outer_dia;

// Gaskets
translate([offset3,0,0])
color("green") 
...
difference() {
  ...
  translate([0,0,-z_fite]) cylinder(gasket_depth+z_fite*2, pipe_fit_dia/2, pipe_fit_dia/2);
}

translate([offset3,0,0])
color("green") 
translate([0,0,middle_section_distance+thread_length+thread_length+offset2]) rotate([180,0,0])
difference() {
   cylinder(gasket_depth, (pipe_fit_dia+gasket_min_thickness+gasket_width*2)/2, (pipe_fit_dia+gasket_min_thickness)/2);
  translate([0,0,-z_fite]) cylinder(gasket_depth+z_fite*2, pipe_fit_dia/2, pipe_fit_dia/2);
}